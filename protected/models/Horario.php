...
class Horario extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_banda, hora_inicio, hora_fin, dia', 'required'),
			array('id_banda', 'numerical', 'integerOnly'=>true),
			array('hora_inicio, hora_fin', 'length', 'max'=>5),
			array('dia', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_horario, id_banda, hora_inicio, hora_fin, dia', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'registroHorarios' => array(self::HAS_MANY, 'RegistroHorario', 'id_horario'),
		);
	}
